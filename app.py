...
class LoadBalancer (object):
    ...
    def handle_arp(self, packet, event):
        ...
        if arp_packet.opcode == pkt.arp.REQUEST:
            ...
            chosen_server_mac = EthAddr("00:00:00:00:00:01")
    ...
    def handle_ip(self, packet, event):
        ...
        if packet.next.dstip == switch_ip:
            if client_ip in client_server_map:
                ...
                client_server_map[client_ip] = (chosen_server_ip, chosen_server_mac)
            
            log.info(f"Forwarding IP packet from {packet.next.srcip} to {chosen_server_ip}")
